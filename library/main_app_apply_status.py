...
def _runtime_running_detail(window: Any) -> str:
    ...
    return f"PID {pid}" if pid else "Monitor is running."


def _update_known_status_widgets(window: Any, title: str, detail: str) -> None:
    subtitle = f"{title} · {detail}" if detail else title
    for attr in ("process_status_row", "runtime_status_row", "monitor_status_row"):
...
def _set_operation_status(window: Any, title: str, detail: str = "", *, toast: bool = False) -> None:
...
def _restore_operation_status(window: Any) -> None:
    if bool(getattr(window, "_apply_sync_status_active", False)):
...
def _begin_apply_progress(app: Any, window: Any) -> None:
    ...
    window._apply_sync_status_generation = getattr(window, "_apply_sync_status_generation", 0) + 1
    generation = window._apply_sync_status_generation

    for delay, title, detail in APPLY_STEPS:
        def show_step(title: str = title, detail: str = detail, generation: int = generation) -> bool:
    ...
    _set_operation_status(window, APPLY_STEPS[0][1], APPLY_STEPS[0][2], toast=True)


def _poll_monitor_until_running(app: Any, window: Any, *, attempts: int = 20, interval_ms: int = 500) -> None:
    ...
    def check() -> bool:
        ...
        if remaining["value"] <= 0:
            _set_operation_status(window, "Starting monitor…", "Overview auto-refresh will keep checking.")
...
def install_main_app_apply_status(app: Any) -> None:
    ...
    if callable(original_finish_sync):
        def finish_sync_theme_video_with_status(self, theme_name: str, message: str, error: str, *args, **kwargs):
            if error:
                ...
                _set_operation_status(self, "Video synced", message or f"Video synced for {theme_name}.", toast=True)
    ...
    if callable(original_finish_apply):
        def finish_used_theme_video_and_start_with_status(self, new_theme: str, message: str, error: str, *args, **kwargs):
            if error:
                _set_operation_status(self, "Video sync failed; starting monitor…", str(error), toast=True)
            else:
                _set_operation_status(self, "Video sync complete; starting monitor…", message or f"{new_theme} is ready.", toast=True)
