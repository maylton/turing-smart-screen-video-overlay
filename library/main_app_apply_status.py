...
This module patches only the main-app control flow messages.  It does not open
serial ports, sync videos, start/stop processes, or change runtime behavior by
itself; those operations remain owned by the existing runtime/video code.
"""
...
def _update_known_status_widgets(window: Any, title: str, detail: str) -> None:
    ...
    # Polished dashboard builds use labels/cards; keep this deliberately broad
    # but scoped to monitor/apply/status-looking attribute names.
    title_tokens = ("monitor", "runtime", "operation", "apply", "sync", "status")
    detail_tokens = title_tokens + ("subtitle", "detail", "description")
    ...
    if callable(original_finish_stop):
        def finish_monitor_stop_with_status(self, result, error, *args, **kwargs):
            ...
            if error:
                ...
                _set_operation_status(self, "Monitor stopped", detail, toast=False)
            _end_operation_status(app, self)
            return output
