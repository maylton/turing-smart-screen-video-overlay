...
def power_off_and_close_display(
    ...
    *,
    ...
    The shutdown sequence is synchronous and deliberately ordered: stop native
    media, wait for the firmware transition, force brightness zero, call
    ``ScreenOff()``, wait for the power command, and only then close serial.
    """
    if driver is None:
        return
    ...
    # Brightness zero is a visual fallback for firmware that accepts TURNOFF
    # but applies it late or inconsistently.
    try:
