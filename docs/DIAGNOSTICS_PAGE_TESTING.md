...
4. The main app exposes the viewer from Settings:
   ...
   The Diagnostics row should open the inline diagnostics page. If the inline page cannot be built, the standalone viewer is used as a fallback.

## Notes

The diagnostics viewer reads configuration, process state, and USB descriptors without opening the display serial port.
