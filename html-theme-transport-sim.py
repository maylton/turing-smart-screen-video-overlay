...
def parse_args(argv):
    ...
    parser.add_argument(
        ...
        action="store_true",
        help=(
            "Compare every simulated Rev. C block with bytes generated by "
            "the existing production driver using captured in-memory I/O."
        ),
    )
...
def run(args) -> int:
    ...
    if args.rev_c_framing and args.profile != "rev-c-2inch":
        ...
        return 2
    if args.production_parity and not args.rev_c_framing:
        print(
            "--production-parity requires --rev-c-framing",
            file=sys.stderr,
        )
        return 2
    ...
    print(
        "Production parity: "
        + (
            "enabled (driver serializers, captured I/O only)"
    ...
    try:
        while True:
            ...
            if args.production_parity:
                ...
                try:
                    parity_result = compare_with_production_driver(
                        frame,
                        result,
                        protocol_result,
                    )
                    ...
                    parity_valid = parity_result.valid
                    protocol_suffix += (
                        " production="
                        + ("ok" if parity_valid else "FAILED")
                    )
                except Exception as exc:
                    print(
                        f"Production parity failed: {exc}",
                        file=sys.stderr,
                    )
                    return 4
