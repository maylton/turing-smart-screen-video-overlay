...
def _status(ok: bool, good: str = "OK", bad: str = "Needs attention") -> str:
...
class DiagnosticsWindow(Adw.ApplicationWindow):
    def __init__(self, app: Adw.Application):
        ...
        copy_button = Gtk.Button(icon_name="edit-copy-symbolic", tooltip_text="Copy text diagnostics report")
        copy_button.connect("clicked", lambda *_: self.copy_report())
        header.pack_end(copy_button)

        copy_json_button = Gtk.Button(label="JSON", tooltip_text="Copy machine-readable diagnostics JSON")
        ...
        intro = Gtk.Label(
            label=(
                "This page reads configuration, theme metadata, monitor process state, "
                "and USB descriptors without opening the display serial port."
            ),
            xalign=0,
            wrap=True,
        )
    ...
    def _render_payload(self, payload: dict[str, Any]) -> None:
        ...
        self._set_card(self.theme_card, theme_name, f"{_status(theme_ok)} · {preview_text}")
